...
# Now create the formatted table with all portfolios (1-10)
...
kable(formatted_table_1958_62, format = "html", col.names = c("Statistic", paste0(1:10))) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  add_header_above(c(" " = 1, "Portfolios for Estimation Period 1958-62" = 10)) %>%
  row_spec(1, bold = FALSE, italic = FALSE)

# Now create the formatted table for portfolios 11-20
formatted_table_1958_62_11_to_20 <- data.frame(
  Statistic = c(
    "Beta (β_{p,t-1})",
    "s(Beta)",
    "R² (r(Rp, Rm)²)",
...
kable(formatted_table_1958_62_11_to_20, format = "html", col.names = c("Statistic", paste0(11:20))) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed")) %>%
  add_header_above(c(" " = 1, "Portfolios for Estimation Period 1958-62" = 10)) %>%
  row_spec(1, bold = FALSE, italic = FALSE)
...
all_panel_c_results <- lapply(periods, function(period) {
  compute_panel_c_metrics(period$start, period$end, period$label)
})

# Combine the results into one table for Panel C
final_panel_c_results <- do.call(rbind, all_panel_c_results)

# Display the final results
print(final_panel_c_results)
